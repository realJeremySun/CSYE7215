...
public class Customer implements Runnable {
	...
	public void run() {
		...
		synchronized(Simulation.spaceLock) {
			try {
				while(Simulation.currentSpace.get()<=0) Simulation.spaceLock.wait();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			
			//enter the shop
			Simulation.currentSpace.getAndDecrement();
		...
		//wait for the order
		try {
			while(Simulation.readyOrder.get(orderNum) == null)
		...
		Simulation.readyOrder.remove(orderNum);
		...
		synchronized(Simulation.spaceLock) {
			//customer leave
			Simulation.currentSpace.incrementAndGet();
