...
public class Machine {
	...
	public ConcurrentMap<Thread, CookAnItem> allFood; 
	...
	public Machine(String nameIn, Food foodIn, int capacityIn) {
		...
		//YOUR CODE GOES HERE...
		this.allFood = new ConcurrentHashMap<Thread, CookAnItem>();
		this.capacityIn = capacityIn;
		this.currentCapacity = new AtomicInteger(capacityIn);
	...
	public boolean waitFood(Order order) throws InterruptedException {
		if(allFood.isEmpty()) return false;
		else {
		...
		public void run() {
			try {
				...
				synchronized(capacityLock) {
					...
					while(currentCapacity.get()<=0) capacityLock.wait();
				}
